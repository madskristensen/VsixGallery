	...
	public class PackageHelper
	{
		...
		public async Task<Package> ProcessVsix(IFormFile file, string repo, string issuetracker, string readmeUrl, string? manageToken = null)
		{
			...
			try
			{
				...
				// Persist the manage token (hash only) for new extensions.
				if (existing?.TokenHash is null)
				{
					SaveManageInfo(vsixFolder, new ManageInfo { TokenHash = HashToken(effectiveToken) });
				}
